...
**eslintrc.json**
 Put all your .eslintrc rules in this file. A good practice is to create an  [ESLint Shareable Config](https://eslint.org/docs/developer-guide/shareable-configs) and extend it, but its not required:
```js
{
...
Since `standard-engine` uses [`eslint`](https://eslint.org/) under-the-hood, you can
...
* `eslintConfig`: object, the options passed to [ESLint's `CLIEngine`](https://eslint.org/docs/developer-guide/nodejs-api#cliengine)
